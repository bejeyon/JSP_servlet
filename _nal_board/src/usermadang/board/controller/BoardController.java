...
//		dao.updateHit(articleno);
		...
		request.setAttribute("articleno", articleno);	// 글번호 articleno 파라미터에 저장.	 
   		request.setAttribute("pageIndex", pageIndex);	// 페이지번호 pageIndex 파라미터에 저장.
   		request.setAttribute("FreeBoardVO", board);	// 출력할 글의 상세 내용 담은 VO 객체 FreeBoardVO 파라미터에 저장.
