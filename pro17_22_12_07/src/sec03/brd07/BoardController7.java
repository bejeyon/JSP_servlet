...
////과거 코드 여러개의 웹서버에서 중복처리
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response)
		...
		System.out.println("BoardController7 ");
		...
		try {
			...
			if (action == null) {
			...
			} // URL에서 요청명[가장 끝부분] listArticles.do
				...
				// 이동할 JSP 페이지 위치 설정
				nextPage = "/board06/listArticles.jsp";
			} // URL에서 요청명[가장 끝부분] articleForm.do
				...
				if (imageFileName != null && imageFileName.length() != 0) {
					File srcFile = new File(ARTICLE_IMAGE_REPO + "\\" 
							+ "temp" + "\\" + imageFileName);
			...
			// URL에서 요청명[가장 끝부분] modArticle.do 이면 처리할 부분
				...
				if (imageFileName != null && imageFileName.length() != 0) {
					...
					FileUtils.moveFileToDirectory(srcFile, destDir, true);
					;
					File oldFile = new File(ARTICLE_IMAGE_REPO 
									+ "\\" + articleNO + "\\" + originalFileName);
					oldFile.delete();
				} // end if
				PrintWriter pw = response.getWriter();
				// 브라우져에 서블릿으로 직접 출력 hard coding
			...
			} //  URL에서 요청명[가장 끝부분] removeArticle.do 이면 처리할 부분
			else if (action.equals("/removeArticle.do")) {
				// js 함수 fn_remove_article 에서 넘어옴
				int articleNO = Integer.parseInt(request.getParameter("articleNO"));
				// DB에서 게시글 삭제 및 번호 가져오기
				List<Integer> articleNOList = boardService.removeArticle(articleNO);
				// 삭제된 게시글의 업로드한 이미지 파일 삭제
				...
				// doHandle 나가기
				return;
			}//URL에서 요청명[가장 끝부분] replyForm.do 이면 처리할 부분
			else if (action.equals("/replyForm.do")) {
				
				int parentNO = Integer.parseInt(request.getParameter("parentNO"));
				//http session가지고 오기
				session = request.getSession();
				//http session에 부모글 번호 저장
				session.setAttribute("parentNO", parentNO);
				// 이동할 JSP 페이지 위치 설정
				nextPage = "/board06/replyForm.jsp";
			} 	//URL에서 요청명[가장 끝부분] addReply.do 이면 처리할 부분
			else if (action.equals("/addReply.do")) {
				//http session가지고 오기
				session = request.getSession();
				
				//http session 에서 가지고 온 데이터 저장
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (fileItem.isFormField()) {
					...
					if (fileItem.getSize() > 0) {
						...
						String fileName = 
								fileItem.getName().substring(idx + 1);
						System.out.println("파일명:" + fileName);
						// 익스플로러에서 업로드 파일의 경로 제거 후 map에 파일명 저장
						articleMap.put(fileItem.getFieldName(), fileName);
						File uploadFile = 
								new File(currentDirPath + "\\temp\\" + fileName);
